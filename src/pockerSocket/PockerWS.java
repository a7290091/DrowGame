...
public class PockerWS{
	...
	@OnMessage
	public void onMessage(Session userSession, String message) throws InterruptedException, IOException {
...
//				}
				...
				NoticeTimer nt = new NoticeTimer(targetSession,oneMsg,5000);
				nt.start();
				//計時器開始
...
//			roomMsg
			...
			for(Session session:sesset) {
    			...
    			session.getBasicRemote().sendText(jsonObj2.toString());
    		}
		}
		if("sendAns".equals(type)) {
			...
			if(ans.equals(senderMsg)) {
				...
				roomMap.get(sender).getBasicRemote().sendText(new JSONObject(StopMsg).toString());
				//取出已完成人數，並將其+1放回
...
//		Set<String> learoom =  rooms.keySet();
//		for(String roomNo:learoom) {
//			Map<String,Session> room = rooms.get(roomNo);
//			Set<String> names = room.keySet();
//			for(String sender : names) {
//				if(userSession.equals(room.get(sender))) {
//					//取得大廳裡的房間物件
//					Map<String,Session> roomMap = rooms.get(roomNo);
//					//從房間物件取得所有人的名稱跟session
//					Set<String> roomMemSet = roomMap.keySet();
